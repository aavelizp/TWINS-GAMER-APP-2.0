...
export default function Dashboard({ session, onLogout }: DashboardProps) {
  ...
  const [listaEspera, setListaEspera] = useState<
    { id: number; nombre: string; pref: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const canalSuscripcion = supabase
      .channel('sincronizacion-global')
      .on('postgres_changes', { event: '*', schema: 'public' }, (payload) => {
        const tabla = payload.table;
        if (tabla === 'consolas') cargarConsolas();
        if (['ventas', 'gastos', 'pagos_deudas'].includes(tabla))
          calcularCajaHoy();
  ...
  const calcularCajaHoy = async () => {
    ...
    const { data: ventas } = await supabase
      .from('ventas')
      .select('monto_usd, monto_bs, metodo_pago')
    ...
    if (ventas)
      ventas.forEach((v) => {
        if (v.metodo_pago !== 'fiado') {
          vUsd += Number(v.monto_usd);
          vBs += Number(v.monto_bs);
        }
      });
    if (gastos)
      gastos.forEach((g) => {
        gUsd += Number(g.monto_usd);
        gBs += Number(g.monto_bs);
      });
    if (pagos)
      pagos.forEach((p) => {
        pUsd += Number(p.monto_usd);
        pBs += Number(p.monto_bs);
      });

    setCajaHoy({
      usd: (vUsd + pUsd - gUsd).toFixed(2),
      bs: (vBs + pBs - gBs).toFixed(2),
    });
  };

  const cambiarTasa = async () => {
    if (rol !== 'admin')
      return alert('Solo administradores pueden cambiar la tasa.');
    const nuevaTasa = prompt('Ingrese nueva tasa (Bs/$):', tasa.toString());
    if (!nuevaTasa || isNaN(Number(nuevaTasa))) return;
    await supabase
      .from('configuracion')
      .update({ tasa_cambio: Number(nuevaTasa) })
      .eq('id', 1);
  };
  ...
  const obtenerConsolaSugerida = (pref: string) => {
    ...
    disponibles.sort((a, b) => {
      ...
      return (
        new Date(a.ultimo_fin_sesion).getTime() -
        new Date(b.ultimo_fin_sesion).getTime()
      );
    });
    if (pref === 'PS4')
      return disponibles.find((c) => c.tipo === 'PS4') || null;
    else if (pref === 'PS5')
      return disponibles.find((c) => c.tipo === 'PS5') || null;
    else return disponibles.find((c) => c.tipo === 'PS4') || disponibles[0];
  };

  const manejarEstadoConsola = async (
    id: number,
    nuevoEstado: string,
    minutos?: number | null,
    prepago: boolean = false,
    clienteNombre?: string | null
  ) => {
    if (nuevoEstado === 'ocupado') {
      setIntercepcionCliente({
        accion: 'inicio_normal',
        idConsola: id,
        minutos,
        prepago,
      });
    ...
    return manejarEstadoConsolaSimple(id, nuevoEstado);
  };

  const manejarEstadoConsolaSimple = async (
    id: number,
    nuevoEstado: string,
    minutos?: number | null,
    prepago: boolean = false,
    ...
    const actualizacion: any = {
      estado: nuevoEstado,
      tiempo_inicio:
        nuevoEstado === 'ocupado' ? new Date().toISOString() : null,
    ...
    if (nuevoEstado === 'disponible')
      actualizacion.ultimo_fin_sesion = new Date().toISOString();

    const { error } = await supabase
      .from('consolas')
      .update(actualizacion)
      .eq('id', id);
    if (error) alert('Error de BD: ' + error.message);
  };
  ...
  const manejarPausaConsola = async (consola: any) => {
    if (consola.estado === 'ocupado') {
      const { error } = await supabase
        ...
        .update({
          estado: 'pausado',
          tiempo_pausa: new Date().toISOString(),
        })
        .eq('id', consola.id);
      ...
      const nuevoInicioMs =
        new Date(consola.tiempo_inicio).getTime() + tiempoPausadoMs;

      const { error } = await supabase
        .from('consolas')
        .update({
          estado: 'ocupado',
          tiempo_inicio: new Date(nuevoInicioMs).toISOString(),
          tiempo_pausa: null,
        })
        .eq('id', consola.id);
  ...
  const ejecutarMoverConsola = async (origen: any, destino: any) => {
    ...
    const ahoraMs =
      origen.estado === 'pausado' && origen.tiempo_pausa
    ...
    if (origen.minutos_solicitados && origen.minutos_solicitados > 0) {
      ...
      const deudaGenerada =
        Math.ceil((mins / 60) * origen.precio_por_hora * 100) / 100;
    ...
    await supabase
      ...
      .update({
        ...
        minutos_solicitados: nuevosMinutos,
        prepago: origen.prepago,
        cliente_nombre: origen.cliente_nombre,
        cliente_id: origen.cliente_id,
        deuda_acumulada: nuevaDeudaAcumulada,
        tiempo_pausa: null,
      })
      .eq('id', destino.id);
    ...
    await supabase
      ...
      .update({
        estado: 'disponible',
        tiempo_inicio: null,
        minutos_solicitados: null,
        prepago: false,
        cliente_nombre: null,
        cliente_id: null,
        deuda_acumulada: 0,
        tiempo_pausa: null,
        ultimo_fin_sesion: new Date().toISOString(),
      })
      .eq('id', origen.id);
  ...
  const abrirPrepago = (consola: any, clienteNombre?: string) => {
  ...
  const confirmarTiempoPrepago = (minutos: number | null) => {
    ...
    let usd =
      minutos && minutos > 0
        ? ((minutos / 60) * consola.precio_por_hora).toFixed(2)
        : (1 * consola.precio_por_hora).toFixed(2);
    setDatosCobro({
      consola,
      usd,
      bs: (Number(usd) * tasa).toFixed(2),
      tasaBs: tasa,
      esPrepago: true,
      minutosPrepago: minutos,
      clienteNombre: consola.cliente_nombre_temporal,
      cliente_id_real: consola.cliente_id_temporal,
      clienteYaSolicitado: true,
    });
  };

  const abrirModalCobroNormal = (
    idConsola: number,
    usdTotal: string,
    bsTotal: string
  ) => {
    const consola = consolas.find((c) => c.id === idConsola);
    if (Number(usdTotal) <= 0) {
      manejarEstadoConsolaSimple(idConsola, 'disponible');
      return;
    }
    ...
    setDatosCobro({
      ...
      tasaBs: tasa,
      esPrepago: false,
      clienteNombre: consola?.cliente_nombre,
      cliente_id_real: consola?.cliente_id,
      clienteYaSolicitado: true,
    });
  };

  const prepararCobroVentaRapida = (
    carrito: any[],
    totalUSD: number,
    totalBS: number
  ) => {
  ...
  const ejecutarCobroFinal = async (
    metodoPago: string,
    detallesPago?: any,
    clienteIdForzado?: number
  ) => {
    if (!datosCobro) return;
    let montoUSDGuardar = Number(datosCobro.usd),
      montoBSGuardar = Number(datosCobro.bs);
    ...
    const idClienteFinal =
      datosCobro.cliente_id_real || clienteIdForzado || null;
    ...
    if (datosCobro.isVentaRapida) {
      ...
      if (datosCobro.carrito && datosCobro.carrito.length > 0) {
        for (const item of datosCobro.carrito) {
          await supabase
            .from('inventario')
    ...
    const { error } = await supabase
      .from('ventas')
      .insert({
        consola_id: datosCobro.consola.id,
        monto_usd: montoUSDGuardar,
        monto_bs: montoBSGuardar,
        tasa_cambio: tasa,
        metodo_pago: metodoPago,
    ...
    await supabase
      ...
      .update({
        horas_uso_acumuladas:
          Number(datosCobro.consola.horas_uso_acumuladas || 0) + horasJugadas,
      })
      .eq('id', datosCobro.consola.id);

    if (idClienteFinal) {
      const { data: clienteActual } = await supabase
        .from('clientes')
        .select('deuda_usd, horas_jugadas')
        .eq('id', idClienteFinal)
        .single();
      if (clienteActual) {
        let actualizacionCliente: any = {
          horas_jugadas:
            Number(clienteActual.horas_jugadas || 0) + horasJugadas,
        };
        if (metodoPago === 'fiado')
          actualizacionCliente.deuda_usd =
            Number(clienteActual.deuda_usd || 0) + montoUSDGuardar;
        await supabase
          .from('clientes')
    ...
    const nombreClienteFinal =
      datosCobro.clienteNombre || clienteAsignando?.nombre || null;

    if (datosCobro.esPrepago) {
      await manejarEstadoConsolaSimple(
        datosCobro.consola.id,
        'ocupado',
        datosCobro.minutosPrepago,
        true,
        nombreClienteFinal,
        idClienteFinal
      );
    ...
    if (clienteAsignando) {
      removerDeListaEspera(clienteAsignando.id);
  ...
  const asignarClienteListaEspera = (cliente: {
    id: number;
    nombre: string;
    pref: string;
  }) => {
  ...
  const anularSesionError = async (idConsola: number) => {
    ...
    if (window.confirm('⚠️ ¿ANULAR sesión sin cobrar?'))
      await manejarEstadoConsolaSimple(idConsola, 'disponible');
  };
  ...
  const generarCierreCaja = async () => {
    ...
    const { data: inventarioActual } = await supabase
      ...
      .select('*')
      .order('nombre');
    const { data: clientesFiados } = await supabase
      .from('clientes')
      .select('*')
      .gt('deuda_usd', 0)
      .order('nombre');
    ...
    ventas.forEach((v) => {
      ...
      v_usdTotal += usd;
      if (v.consola_id) v_consolas += usd;
      else v_snacks += usd;
    ...
    let g_usdTotal = 0;
    gastos.forEach((g) => {
      g_usdTotal += Number(g.monto_usd);
    });
    const ventasNetasUSD = v_usdTotal - v_fiado + p_recuperado - g_usdTotal;

    setDatosCierre({
      fecha: new Date(inicioJornada).toLocaleDateString('es-VE'),
      ...
      tasa,
      ventasBrutasUSD: v_usdTotal.toFixed(2),
      ventasConsolasUSD: v_consolas.toFixed(2),
      ventasSnacksUSD: v_snacks.toFixed(2),
      ingresoEfectivoUSD: v_efectivoUsd.toFixed(2),
      ingresoPagoMovil: v_pagoMovil.toFixed(2),
      ingresoEfectivoBs: v_efectivoBs.toFixed(2),
      fiadoHoyUSD: v_fiado.toFixed(2),
      recuperadoUSD: p_recuperado.toFixed(2),
      gastosTotalesUSD: g_usdTotal.toFixed(2),
      ventasNetasUSD: ventasNetasUSD.toFixed(2),
      ventasDetalle: ventas,
      gastosDetalle: gastos,
      inventarioDetalle: inventarioActual || [],
      fiadosDetalle: clientesFiados || [],
    });
  };
  ...
  const calcularDeudaConsola = (consola: any) => {
    ...
    const diff = Math.floor(
      (ahoraMs - new Date(consola.tiempo_inicio).getTime()) / 1000
    );
    const secs = diff > 0 ? diff : 0;
    const esFijo = Boolean(
    ...
    const extra = Math.ceil(
      (esFijo && restantes <= 0 ? Math.abs(restantes) : 0) / 60
    );
    ...
    if (consola.prepago) deudaBase = (extra / 60) * consola.precio_por_hora;
      ...
      deudaBase =
        ...
        (extra / 60) * consola.precio_por_hora;
    else deudaBase = (Math.ceil(secs / 60) / 60) * consola.precio_por_hora;

    return deudaBase + (Number(consola.deuda_acumulada) || 0);
  };

  const ejecutarCobroMultipleFinal = async (
    metodoPago: string,
    detallesPago: any,
    consolasIds: number[]
  ) => {
    let sumaDeudasGlobal = 0;
    const matrizDeudas = consolasIds.map((id) => {
      const deuda = calcularDeudaConsola(consolas.find((c) => c.id === id));
      sumaDeudasGlobal += deuda;
      return { id, deuda };
    });
    for (const item of matrizDeudas) {
      ...
      const peso =
        sumaDeudasGlobal > 0
          ? item.deuda / sumaDeudasGlobal
          : 1 / consolasIds.length;
      if (metodoPago.startsWith('mixto')) {
        mUsd = detallesPago.monto_usd * peso;
        mBs = detallesPago.monto_bs * peso;
      } else if (metodoPago === 'efectivo_bs' || metodoPago === 'pago_movil')
        mBs = item.deuda * tasa;
      else mUsd = item.deuda;
      ...
      if (consolaCompleta) {
        ...
        await supabase
          ...
          .update({
            horas_uso_acumuladas:
              Number(consolaCompleta.horas_uso_acumuladas || 0) + horasJugadas,
          })
          .eq('id', item.id);
      }
  ...
  const agregarAListaEspera = () => {
    if (!nombreEspera.trim()) {
      setErrorEspera(true);
      return;
    }
    setListaEspera([
      ...listaEspera,
      { id: Date.now(), nombre: nombreEspera, pref: prefEspera },
    ]);
    setNombreEspera('');
    setErrorEspera(false);
  };
  const removerDeListaEspera = (id: number) =>
    setListaEspera(listaEspera.filter((c) => c.id !== id));
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            {/* AQUÍ REEMPLAZAMOS EL ALIEN POR EL LOGO DE LA EMPRESA */}
            <img
              src="/logo-twins.jpg"
              alt="Twins Gamer"
              style={{
          ...
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
            ...
            >
              <span style={{ fontSize: '11px', color: '#887bb0' }}>
                TASA BS/$
              </span>
            ...
            {(rol === 'admin' || rol === 'cajero') && (
              <div style={{ textAlign: 'center', marginRight: '10px' }}>
                ...
                <strong style={{ color: '#fff', fontSize: '14px' }}>
                  ${cajaHoy.usd}{' '}
                  <span style={{ color: '#ff007f' }}>- Bs {cajaHoy.bs}</span>
                </strong>
              </div>
            )}
            <button
            ...
            <button
              onClick={() => setMostrarVentaRapida(true)}
              style={{
                background: 'rgba(11, 8, 21, 0.7)',
            ...
            <button
              onClick={generarCierreCaja}
              style={{
                background:
                  'linear-gradient(90deg, rgba(155,81,224,0.9), rgba(161,38,255,0.9))',
                border: 'none',
                color: '#fff',
                padding: '8px 15px',
                borderRadius: '8px',
        ...
        {pestañaActiva === 'consolas' && (
          <main style={{ padding: '20px', display: 'flex', gap: '20px' }}>
                ...
                {consolas.map((c) => {
                  ...
                  return (
                    <TarjetaConsola
                      ...
                      tasaBs={tasa}
                      rol={rol}
                      esSugerida={sugeridaParaTipo?.id === c.id}
                      onActualizarEstado={manejarEstadoConsola}
                      onAbrirPrepago={abrirPrepago}
                      onCobrar={abrirModalCobroNormal}
                      onAnular={anularSesionError}
                      onMover={(consola: any) => setConsolaAMover(consola)}
                      onPausar={manejarPausaConsola}
                    />
                  );
                })}
              </div>
            ...
            <div style={{ flex: 1, minWidth: '280px' }}>
              <button
                onClick={() => setMostrarCobroMultiple(true)}
                style={{
                  width: '100%',
              ...
              >
                ...
                >
                  <span style={{ color: '#ff007f' }}>👥</span> Lista de Espera
                </h3>
                <input
                  type="text"
                  placeholder="Nombre"
                  value={nombreEspera}
                  onChange={(e) => {
                    setNombreEspera(e.target.value);
                    setErrorEspera(false);
                  }}
                  style={{
                    width: '100%',
                    background: 'rgba(11, 8, 21, 0.8)',
                    border: `1px solid ${errorEspera ? '#ef4444' : '#251b45'}`,
                    color: '#fff',
                    padding: '10px',
                    borderRadius: '8px',
                    marginBottom: errorEspera ? '5px' : '15px',
                ...
                >
                  {['PS4', 'PS5', 'Cualquiera'].map((pref) => (
                    <button
                      key={pref}
                      onClick={() => setPrefEspera(pref)}
                      style={{
                        background:
                          prefEspera === pref
                            ? '#9b51e0'
                            : 'rgba(11, 8, 21, 0.8)',
                        border:
                          prefEspera === pref ? 'none' : '1px solid #3c2a7a',
                        color: prefEspera === pref ? '#fff' : '#887bb0',
                        padding: '8px 0',
                        borderRadius: '8px',
                        fontSize: '11px',
                        cursor: 'pointer',
                ...
                {listaEspera.length === 0 ? (
                  <span style={{ fontSize: '12px', color: '#55497a' }}>
                      ...
                      >
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div style={{ color: '#887bb0', fontSize: '11px' }}>
                              {c.pref}
                            </div>
                          </div>
                        </div>
                        <div style={{ display: 'flex', gap: '5px' }}>
                          <button
                            onClick={() => asignarClienteListaEspera(c)}
                            style={{
                              background: '#a126ff',
                              border: 'none',
                              color: '#fff',
                          ...
                          <button
                            onClick={() => removerDeListaEspera(c.id)}
                            style={{
                              background: 'transparent',
                              border: 'none',
                              color: '#fff',
        ...
        {pestañaActiva === 'mantenimiento' && (
          <TabMantenimiento consolas={consolas} onRecargar={() => {}} />
        )}
        {pestañaActiva === 'ventas_hoy' && (
          <TabVentasHoy tasa={tasa} rol={rol} />
        )}
      </div>
      ...
      {intercepcionCliente && (
        <ModalAsignarCliente
          ...
          onConfirm={(clienteId: number | null, nombre: string) => {
            const { accion, idConsola, consola, minutos, prepago } =
              intercepcionCliente;
            setIntercepcionCliente(null);
            if (accion === 'inicio_normal')
              manejarEstadoConsolaSimple(
                idConsola,
                'ocupado',
                minutos,
                prepago,
              ...
              setConsolaPrepago({
                ...consola,
                cliente_nombre_temporal: nombre,
                cliente_id_temporal: clienteId,
              });
          }}
        />
      )}
      ...
      <ModalPrepago
        consola={consolaPrepago}
        onClose={() => {
          setConsolaPrepago(null);
          setClienteAsignando(null);
        }}
        onConfirmarTiempo={confirmarTiempoPrepago}
      />
      <ModalCobro
        datos={datosCobro}
        onClose={() => {
      ...
      {mostrarCobroMultiple && (
        <ModalCobroMultiple
          consolas={consolas}
          tasa={tasa}
          calcularDeuda={calcularDeudaConsola}
          onClose={() => setMostrarCobroMultiple(false)}
          onConfirm={ejecutarCobroMultipleFinal}
        />
      )}
      {mostrarVentaRapida && (
        <ModalVentaRapida
          tasa={tasa}
          onClose={() => setMostrarVentaRapida(false)}
          onConfirm={prepararCobroVentaRapida}
        />
      )}
      {mostrarGastos && (
        <ModalGastos
